...
main() {
  ...
  case "$OS_TYPE" in
    Darwin)
      ...
      ;;
    Linux|MINGW64_NT*|MINGW32_NT*|MSYS_NT*)
      case "$command" in
        start)   linux_direct_start ;;
        stop)    linux_direct_stop ;;
        restart) linux_direct_stop; sleep 1; linux_direct_start ;;
        status)  linux_direct_status ;;
        logs)    linux_logs ;;
        *)
          echo "Usage: daemon.sh {start|stop|restart|status|logs}"
          echo "Platform: $OS_TYPE (direct mode)"
          exit 1
          ;;
      esac
      ;;
